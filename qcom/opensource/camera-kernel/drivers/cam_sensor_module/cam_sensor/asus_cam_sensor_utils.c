...
{
	{SENSOR_ID_IMX766, "IMX766", 50, 0x0766},
	{SENSOR_ID_IMX663, "IMX663", 12, 0x0663},
	...
	for(i=0;i<=MAX_CAMERA_ID;i++)
	{
		if(g_sensor_ctrl[i] && g_sensor_ctrl[i]->sensordata->subdev_id[sub_module]!= -1)
		{
			if(g_sensor_ctrl[i]->sensordata->subdev_id[sub_module] == index)
			{
				...
				*camera_id = i;
				pr_err("[eeprom_debug] got camera id %d for submodule %s, index %d sensor_probe_addr_type =0x%x\n",
						*camera_id,
