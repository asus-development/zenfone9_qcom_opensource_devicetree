...
uint8_t g_cam_csi_check_count = 0;  //ASUS_BSP "Add for camera csi debug count"
...
#elif defined ASUS_AI2202_PROJECT
static eeprom_group_t  g_eeprom_group[] =
{
  //MODULE_ID  SENSOR_NAME
  {0x79,       "IMX766_D"},
...
static struct proc_ops camera_module_fops = {
	//.owner = THIS_MODULE,
...
static struct proc_ops camera_resolution_fops = {
	//.owner = THIS_MODULE,
	.proc_open = resolution_proc_open,
	.proc_read = seq_read,
	.proc_lseek = seq_lseek,
	.proc_release = single_release,
};
...
static struct proc_ops camera_status_fops = {
	//.owner = THIS_MODULE,
...
static const struct proc_ops sensor_i2c_debug_fops = {
	//.owner = THIS_MODULE,
	.proc_open = sensor_i2c_debug_open,
	.proc_read = seq_read,
	.proc_write = sensor_i2c_debug_write,
	.proc_lseek = seq_lseek,
	.proc_release = single_release,
};
...
static const struct proc_ops eeprom_i2c_debug_fops = {
	//.owner = THIS_MODULE,
	.proc_open = eeprom_i2c_debug_open,
	.proc_read = seq_read,
	.proc_write = eeprom_i2c_debug_write,
	.proc_lseek = seq_lseek,
	.proc_release = single_release,
};
...
static sensor_define_t project_camera[MAX_CAMERA_ID+1] = {
	{SENSOR_ID_IMX766, DIRECTION_REAR},
	{SENSOR_ID_IMX663, DIRECTION_FRONT},//ASUS_BSP Ryan add for cam 1
...
static struct proc_ops sensors_res_fops = {
	//.owner = THIS_MODULE,
	.proc_open = sensors_res_proc_open,
	.proc_read = seq_read,
	.proc_lseek = seq_lseek,
	.proc_release = single_release,
};
...
*/
	...
	****************************/
	if((camera_id == 0 || camera_id == 2) && !g_otp_data_override_done[camera_id])
	{
		...
		}else if ( g_eeprom_info[0].mapdata[MODULE_ID_ADDR] != 0x74 ){ //Check 13M (camera_id == 2)Module ID:"0x74", addr:"7213"  in EEPROM 0
...
static struct proc_ops otp_fops = {
	//.owner = THIS_MODULE,
	.proc_open = otp_proc_open,
	.proc_read = seq_read,
	.proc_lseek = seq_lseek,
	.proc_release = single_release,
};
...
static struct proc_ops eeprom_proc_fops = {
	//.owner = THIS_MODULE,
	.proc_open = eeprom_proc_open,
	.proc_read = seq_read,
	.proc_lseek = seq_lseek,
	.proc_release = single_release,
};
...
static struct proc_ops eeprom_to_otp_fops = {
	//.owner = THIS_MODULE,
	.proc_open = eeprom_to_otp_open,
	.proc_read = seq_read,
	.proc_lseek = seq_lseek,
	.proc_release = single_release,
};
...
//ASUS_BSP +++ "Add for camera vcm data"
static int camera_vcm_data_read(struct seq_file *buf, void *v)
{
	eeprom_info_t *eeprom = (eeprom_info_t *)buf->private;
	if(!eeprom->mapdata)
	{
	...
	{
		seq_printf(buf, "0x%02X 0x%02X 0x%02X 0x%02X\n", eeprom->mapdata[0x18], eeprom->mapdata[0x19], eeprom->mapdata[0x1A], eeprom->mapdata[0x1B]);
	}
	return 0;
}


static int camera_vcm_data_open(struct inode *inode, struct	 file *file)
{
	return single_open(file, camera_vcm_data_read, PDE_DATA(inode));
}

static struct proc_ops cam_vcm_data_fops = {
	//.owner = THIS_MODULE,
	.proc_open = camera_vcm_data_open,
	.proc_read = seq_read,
	.proc_lseek = seq_lseek,
	.proc_release = single_release,
};
...
void eeprom_dump_create(struct cam_eeprom_ctrl_t * e_ctrl, uint8_t moduleGroupIndex)
{
	...
	//pr_info("[DIT_EEPROM]  g_eeprom_ctrl[moduleGroupIndex] = %p,  %p", g_eeprom_ctrl[moduleGroupIndex], &g_eeprom_ctrl[moduleGroupIndex]);
	...
	if(g_eeprom_ctrl[moduleGroupIndex] != NULL)//if loaded. free it first
	{
		...
		remove_proc_entry(PROC_VCM_DATA, NULL);
	...
	if(g_eeprom_ctrl[moduleGroupIndex] == NULL)//first time
	{
		...
		{
			create_proc_file(PROC_VCM_DATA, &cam_vcm_data_fops, &g_eeprom_info[moduleGroupIndex]);
		}
...
static struct proc_ops cam_csi_check_fops = {
	.proc_read  = cam_csi_check_read,
	.proc_write = cam_csi_check_write,
};
...
//ASUS_BSP +++ "Add for camera csi debug count"
static ssize_t cam_csi_check_count_read(struct file *file, char __user *buf, size_t count, loff_t *ppos)
{
	int len = 0;
	ssize_t ret = 0;
	char *buff;
	...
	pr_err("camera csi check count %d\n", g_cam_csi_check_count);
...
static struct proc_ops cam_csi_check_count_fops = {
	.proc_read  = cam_csi_check_count_read,
};

static void cam_csi_debug_count_create(void)
{
	create_proc_file(PROC_CSI_CHECK_COUNT, &cam_csi_check_count_fops, NULL);
}
//ASUS_BSP --- "Add for camera csi debug count"
...
static struct proc_ops cam_cci_check_fops = {
	.proc_read  = cam_cci_check_read,
	.proc_write = cam_cci_check_write,
};
...
void asus_cam_sensor_init(struct cam_sensor_ctrl_t *s_ctrl)
{
	...
	pr_err("CHHO  CAMERA ID %d init :E", s_ctrl->id);
	if(!s_ctrl)
	{
		pr_err("s_ctrl is NULL\n");
		return;
